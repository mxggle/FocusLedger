...
import { streamChat, streamChatV2 } from "./chatClient";
...
describe("streamChatV2", () => {
  ...
  it("handles malformed OpenAI tool arguments gracefully as {}", async () => {
    fetch.mockResolvedValue(
      sseResponse([
    ...
    expect(text).toBe("");
    expect(toolCalls).toEqual([{ name: "list_tasks", args: {} }]);
  });
